...
class UnitPayModel
{
    ...
    public function confirmPaymentByUnitpayId($unitpayId)
    {
        ...
        $result = $this->getPaymentByUnitpayId($unitpayId);

        // $query = "UPDATE `mstu_tvrs` SET `count` = "
        $query = "UPDATE `transactions` SET `status` = 1, `unitpayId` = '$unitpayId' WHERE `id` = $result->account ";
