...
abstract class DbSimple_Database extends DbSimple_LastError
{
    ...
    **/
	...
	/**
     * Расширени ф-ии query для вставки
	 * mixed insert(string $table , array $row)
     * $row - ассоциативный массив поле-значение
     */
	 
    public function insertRow($table, $row)
    {
		return $this->query("INSERT INTO $table(?#) VALUES(?a)",array_keys($row), array_values($row));
    }
	
	/**
	 * Обновляет, если $unicfield!=0
	 * mixed set(string $table , array $row)
	 * $row - ассоциативный массив поле-значение
	 * $unicfield - первичный ключ
	 */
	
    public function updateRow($table, $row, $unicfield='id')
    {
		if(isset($row[$unicfield]) && ($row[$unicfield])>0)
			return $this->query("UPDATE `$table` SET ?a WHERE `$unicfield`=? LIMIT 1",$row,$row[$unicfield]);
		else return false;
    }	
	
	/**
     * Обновляет или добавляет, если $unicfield=0
	 * mixed set(string $table , array $row)
     * $row - ассоциативный массив поле-значение
     * $unicfield - название поля с первичным ключом (числовое)
     */
	
    public function set($table, $row, $unicfield='id')
    {
		if(($row[$unicfield])>0)
			return $this->query("UPDATE `$table` SET ?a WHERE `$unicfield`=?d LIMIT 1",$row,$row[$unicfield]); 
		else {
			unset($row[$unicfield]);
			return $this->query("INSERT INTO $table(?#) VALUES(?a)",array_keys($row), array_values($row));
		}
    ...
    private function _query($query, &$total)
    {
		...
		// $this->lastQuery=$this->_performQuery($query);
		// $this->lastQuery=$this->_performFetch($result);
        $this->_resetLastError();
