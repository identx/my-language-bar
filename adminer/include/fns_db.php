<?php //дб
include("config.php");
// echo (file_exists("include/config.php"))?'true':'false';

function accessed($__FILE__){
	global $_CONFIG;
	$_cp=explode($_CONFIG['dir'],$__FILE__); 
	$_cp=$_cp[count($_cp)-3];
	if (!isset($_SESSION["m_user"]["id"]) || (array_search($_cp,$_SESSION["m_user"]["has"])===false)) return false;
	return true;
}
...
class simpleMysqli extends simpleDB{
	public function __construct($connectionSettingsArray){
		...
		if($this->db_resource->connect_errno)
			throw new Exception($this->db_resource->connect_errno);			
	}
    ...
    public function error(){
		return $this->db_resource->connect_error;
	}
    public function errno(){
		return $this->db_resource->connect_errno;
	}
