...
/** Magic method which allows read-only access to protected or private class properties
  ...
  * @return mixed */
    ...
    protected function imageResize($image, $file=null) {
        ...
        // RESIZE TO FIT
        } elseif ($this->config['maxImageWidth'] && $this->config['maxImageHeight'] &&
    ...
    protected function makeThumb($file, $overwrite=true) {
        ...
        if (($img->width <= $this->config['thumbWidth']) &&
            ...
            if (in_array($type, array("gif", "jpeg", "png")))
                return true;
        // Resize image
        // } elseif (!$img->resizeCrop($this->config['thumbWidth'], $this->config['thumbHeight']))
        } else{
			if($this->config['method']=='crop'){
				if(!$img->resizeCrop($this->config['thumbWidth'], $this->config['thumbHeight']))
					return false;
