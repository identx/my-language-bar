...
function tvrs_snippet($one,$attrs){
	global  $conf;
	...
	return '<div class="tvrs_snippet '.($one['opts']&1==1?'alpha':'').'">
			<a href="tools.php?p=tovars&id='.$one['id'].'" >
				...
				<span class="">Преподает: '.$attrs[2]["t5"].'</span><br>
				<span class="">Говорит на: '.$attrs[3]["t5"].'</span>
			</a>
			</div>';
}
...
if(!isset($_GET['id'])){
	...
	$TAG['content'].='<form action="" method="get" id="tvrs_search"><input name="search" class="text short" placeholder="поиск по имени" value="'.$_GET['search'].'"/><input type="hidden" name="p" value="tovars"/> <input type="submit" class="btn ui-button ui-state-default" value="Искать"/>
	</form>

	<br />
	<a class="btn ui-button ui-state-default" class="tvr_addnew" href="?p=tovars&id=0&tmpl=1&cat='.$_GET['cat'].'">+ Добавить</a>
	<br />
	';
	...
	$sql="
		SELECT `"._DB_PREF_."tvrs`.*, `"._DB_PREF_."images`.`name` AS 'img' 
		FROM `"._DB_PREF_."tvrs`
	...
	foreach($res as $one){
		...
		FROM `"._DB_PREF_."attrs`
			...
			INNER JOIN `"._DB_PREF_."tvrs` ON `"._DB_PREF_."templ_attrs`.`templ`=$one[templ]
		...
		WHERE `"._DB_PREF_."tvrs`.`id`=$one[id] ORDER BY `group`";
	    

	    $attrs=db_getItems($sql);	
		$TAG['content'].=tvrs_snippet($one,$attrs);
	}
	...
	$TAG['content'].='<br /><div class="tvrs_category" style="display: none;">Категория<br /><select name="cat" id="tvrs_category_item" class="def_val superselect">'.tvrs_options($nds,$tovar['cat']).'</select></div>';
	$TAG['content'].='<div class="tvrs_category" style="display: none;">Шаблон<br /><select id="tvrs_category2" class="def_val superselect" name="templ">';
	foreach($tmps as $one){
	...
	$TAG['content'].='	<div class="element_wrapper cb">ФИО<input class="def_val" type="text" id="tvr_name" data-alias="tvr_alias" name="name" value="'.htmlspecialchars($tovar['name']).'"/></div>';

	$TAG['content'].='<div class="element_wrapper cb short">Видео<input class="def_val" type="text" id="tvr_alias" name="alias" value="'.$tovar['alias'].'"/></div>';
...
//	$TAG['content'].='<div class="def_val element_wrapper short">Цена со скидкой<input type="text" class="def_val" name="price_d" value="'.htmlspecialchars($tovar['price_d']).'"/></div>';
	$TAG['content'].='<div class="def_val element_wrapper short">Часовой пояс<input type="text" name="count" class="spinner def_val" value="'.htmlspecialchars($tovar['count']).'"/></div>';
	$TAG['content'].='<div class="def_val element_wrapper short cb">Скайп<input type="text" class="def_val" name="article" value="'.htmlspecialchars($tovar['article']).'"/></div>';
//	$TAG['content'].='<div class="def_val element_wrapper shortest">Скрыть<br /><select class="'.(($tovar['opts'] & 1==1)?' attention':'').'" id="sel_hide" name="hide"><option value="0" >Нет</option><option '.(($tovar['opts'] & 1==1)?'selected="selected"':'').' value="1">Да</option></select></div>';
	$TAG['content'].='<div class="element_wrapper short" id="brander">Страна<br /><select class="def_val" name="brand"><option data-img="images/noimage.png"	value="0">не выбрано</option>';
