...
/*
	...
	showHiddens($value)	 - вкл/выкл отображение скрытых товаров
...
class Catalog{
	...
	public function imgAdd($id_tovar,$imgs){
		...
		foreach($imgs as $img){
			...
			if($ok){					
				$iid=db_query("INSERT INTO `".$this->pr."images` (`id`,`id_tvr`,`index`,`name`,`caption`) VALUES ('','$id_tovar','$index','$iid.$ext','')");
			}
			$index++;
		}
		return $iid;
	...
	public function search($str){
		...
		WHERE `price`>-1 AND `cat`>0 ".$this->show_hidden." HAVING `relevant`>".(round(mb_strlen(implode($trueWords))*($this->relevant/100)))." ORDER BY `relevant`,`id` DESC";
		$res=db_query($query);
		$this->setLog($query);
	...
	public function getItems($filter=array()){
		...
		if(count($this->items_filter_attrs)==0){
			$q="SELECT `".$this->pr."tvrs` .*, (SELECT `name` FROM `".$this->pr."images` WHERE `id_tvr`=`".$this->pr."tvrs`.`id` ORDER BY `index` ASC LIMIT 1) as 'img'  FROM `".$this->pr."tvrs` WHERE `count`>0 ".$this->items_filter_params." ".$this->show_hidden.$this->items_sort.$this->items_limit_string;
			$res=db_query($q);
			$this->setLog($q);
			
			if($this->items_limit_string!=''){
				$q="SELECT COUNT(*) `c` FROM `".$this->pr."tvrs` WHERE `count`>0 ".$this->items_filter_params." ".$this->show_hidden;
				$cnt=db_query($q);
				$this->setLog($q);
			...
			$q="SELECT `id` FROM `".$this->pr."tvrs` WHERE `count`>0 ".$this->items_filter_params." ".$this->show_hidden;
			$ids=db_query($q);
			$this->setLog($q);
			...
			$q="SELECT `".$this->pr."tvrs` .*, (SELECT `name` FROM `".$this->pr."images` WHERE `id_tvr`=`".$this->pr."tvrs`.`id` AND `".$this->pr."images`.`index`=0) as 'img'  FROM `".$this->pr."tvrs` 	
	...
	public function getItem($id){
		$q="SELECT * FROM `".$this->pr."tvrs` WHERE `count`>=0 AND `id`='$id' ".$this->show_hidden."  LIMIT 1";
	...
	public function showHiddens($value){
		if($value)
			$this->show_hidden='';
		else 
			$this->show_hidden='AND (`opts`&1=0)';
		return $this;
	}
