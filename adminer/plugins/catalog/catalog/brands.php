...
if(!isset($_GET['id'])){
	...
	$TAG['content'].='<form id="brand_editor" data-tid="'.$ID.'">';
		...
		<div class="attribute"><label>Ссылка</label><input id="brand_url" type="text" placeholder="название" class="attr_val" value="'.htmlspecialchars($brand['url']).'"/></div>		
		<div class="attribute"><label>Описание</label><textarea class="attr_val type_6" id="attr_brdescr" >'.$brand['descr'].'</textarea></div>		
		</div>';
		$TAG['content'].='
