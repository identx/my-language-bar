...
function tvrs_snippet($one){
	...
	return '<div class="tvrs_snippet '.($one['opts']&1==1?'alpha':'').'">
			...
			<div class="sn_extra">'.$one['price'].' руб.<br>
				'.$one['price_d'].' руб.<br>
				Кол-во уроков '.$one['name'].'.<br>
			</div></div>';
}
...
$TAG['content'].='<br /><div style="display: none;" class="tvrs_category">Категория<br /><select name="cat" id="tvrs_category_item" class="def_val superselect">'.tvrs_options($nds,$tovar['cat']).'</select></div>';
...
$TAG['content'].='</select> &nbsp; <a href="#" id="tovar_retempl" data-href="?p=tovars&id='.$ID.'&cat='.$tovar['cat'].'&tmpl=" class="fr btn ui-button ui-state-default">Cменить шаблон</a> </div>';
	...
	$TAG['content'].= '<div class="tvrs_category">Категория<br /><select name="cat" id="tvrs_category_item" class="def_val superselect">'.tvrs_options($nds,$tovar['cat']).'</select></div>';
	$TAG['content'].='	<div class="element_wrapper cb">Кол-во уроков<input class="def_val" type="text" id="tvr_name" data-alias="tvr_alias" name="name" value="'.htmlspecialchars($tovar['name']).'"/></div>';
